...
import { getEMRByAppointmentID, searchMedicines, savePrescription } from '../../ApiClient/prescriptionService';
...
function CreatePrescription() {
    ...
    useEffect(() => {
        const fetchPatientData = async () => {
            try {
                const patientInfo = await getEMRByAppointmentID(parseInt(appointmentId));
                setPatientData(patientInfo);
                // Generate prescription code
                setPrescriptionCode('P-' + Date.now().toString().substring(7));
            } catch (err) {
    ...
    const handleSaveFinalPrescription = async (e) => {
        e.preventDefault();
        setSaving(true);
        setError('');
    ...
    return (
        <div className="consultation-content">
                ...
                <div className="card">
                    <h2>Create New E-Prescription</h2>
                    {error && (
                    ...
                    <form className="add-med-form" onSubmit={(e) => { e.preventDefault(); handleAddToPrescription(); }}>
                        <div className="form-row">
                            <div className="form-group" style={{ flex: 2, position: 'relative' }}>
                                <label htmlFor="medSearch">1. Search & Select Medicine</label>
                                <input
                                    type="text"
                                    id="medSearch"
                        ...
                        <div className="form-group">
                            <label htmlFor="medNotes">3. Dosage & Notes (e.g., 1 tablet twice a day)</label>
                            <textarea
                                id="medNotes"
                                rows="2"
                                value={medicineNote}
                    ...
                    <h2 style={{ marginTop: '30px', marginBottom: '20px' }}>Current Prescription (ID: {prescriptionCode})</h2>
                    <table>
                        <thead>
                            <tr>
                                <th>Medicine</th>
                    ...
                    <form onSubmit={handleSaveFinalPrescription} style={{ marginTop: '30px' }}>
                        <div className="form-actions">
                            <button type="submit" className="save-btn" disabled={saving}>
                                {saving ? 'Đang lưu...' : 'Save Final Prescription'}
                            </button>
                        </div>
                    </form>
                </div>
